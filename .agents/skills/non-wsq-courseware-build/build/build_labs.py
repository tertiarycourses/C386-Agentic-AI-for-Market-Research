...
for lab in labs:
    ...
    if lab.get("challenge"):
        L.append("## Optional Challenge (+10 minutes, outside core duration)\n")
        L.append(lab["challenge"] + " Save the result in the numbered lab output.\n")
    if lab.get("reflection"):
        L.append("## Reflection\n")
        L.append(lab["reflection"] + " Record your answer in the numbered lab output.\n")
    L.append("---\n")
...
lab_minutes=sum(int(lab.get("duration","0 minutes").split()[0]) for lab in labs)
...
if getattr(C,"LABS_SETUP",None):
    R.append("\n## Common Setup\n")
    for item in C.LABS_SETUP:
        R.append(f"- {item}")
if getattr(C,"REJOIN_PATHS",None):
    R.append("\n## Rejoin Path\n")
    R.append("If you missed an earlier lab, rerun the prerequisite lab or ask the trainer for the matching checkpoint pack. Validate it below before continuing; never invent or skip a result.\n")
    R.append("| Join at lab | Required prior file(s) | Validate before continuing |")
    R.append("|---|---|---|")
    for lab_num, files, check in C.REJOIN_PATHS:
        R.append(f"| {lab_num} | `{files}` | {check} |")
for t in C.TOPICS:
    R.append(f"\n## Topic {t['num']} — {t['title']}\n")
