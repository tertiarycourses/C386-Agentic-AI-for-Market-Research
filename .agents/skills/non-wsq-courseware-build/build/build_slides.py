...
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    rect(s,Inches(0.85),Inches(4.25),Inches(11.7),Inches(2.25),_panel())
    txt(s,Inches(1.1),Inches(4.42),Inches(11),Inches(0.3),[[("You'll build",13,_acc(BLUE),True)]])
    bsize=16 if len(str(build)) <= 170 else 14
    txt(s,Inches(1.1),Inches(4.73),Inches(11),Inches(0.88),[[(build,bsize,_ink(),True)]],space=2)
    txt(s,Inches(1.1),Inches(5.82),Inches(11.2),Inches(0.42),[[("Tools:  ",12,_grey(),True),(services,12,_grey(),False)]]); footer(s); return s
...
for t in C.TOPICS:
    ...
    # Course-local concept-first extension. Each teaching section becomes a
    # prose explanation followed by a visual framework, so the deck teaches
    # the idea before the connected activity applies it.
    for sec in t.get("teaching_sections", []):
        content(sec["title"], sec.get("paragraphs", []),
                kicker=f"TOPIC {t['code']} · CONCEPT", size=15)
        if sec.get("visual"):
            tile_grid(sec["title"] + " — Concept Map", sec["visual"],
                      kicker=f"TOPIC {t['code']} · VISUAL FRAMEWORK",
                      cols=2, size=14, accent=accent)
    ...
    for a in acts:
        ...
        _lab_extras(a["num"])
        # Full click-level detail remains in the Learner Guide and lab files.
        # The deck carries only high-level phases supplied by the same source
        # record, preserving alignment while keeping concepts dominant.
        deck_steps=a.get("slide_steps", a["steps"])
        steps=[(si,sc) for (si,sc) in deck_steps if "youtube" not in sc.lower() and "youtube" not in si.lower()]
        total=len(steps)
