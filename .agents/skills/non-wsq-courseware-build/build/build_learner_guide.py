...
for t in C.TOPICS:
    ...
    for a in [x for x in ACT if x["topic"]==t["num"]]:
        ...
        p(a["test"])
        if a.get("checkpoint"):
            h3("Checkpoint for the next lab")
            p(a["checkpoint"])
        if a.get("troubleshooting"):
            h3("Troubleshooting")
            bullets([f"{problem}: {fix}" for problem, fix in a["troubleshooting"]])
        if a.get("challenge"):
            h3("Optional challenge (+10 minutes, outside core duration)")
            p(a["challenge"] + " Save the result in the numbered lab output.")
        if a.get("reflection"):
            h3("Reflection")
            p(a["reflection"] + " Record your answer in the numbered lab output.")
        note(f"Full lab instructions are in labs/lab-{a['num']:02d}-*.md. "
...
for kind,*rest in B:
    if kind=="h1": doc.add_heading(rest[0],level=1)
        ...
        for term,defn in rest[0]:
            ...
            r=para.add_run(term+" — "); r.bold=True; r.font.size=Pt(9)
            d=para.add_run(defn); d.font.size=Pt(9)
